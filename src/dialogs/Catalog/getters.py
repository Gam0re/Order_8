...
async def get_level_3(dialog_manager: DialogManager, **middleware_data):
     async with async_session() as session:
        ...
        db_main_items = set(await session.scalars(select(Catalog).where(Catalog.level_2 == lvl2_name and Catalog.level_3 == '')))
...
async def get_level_4(dialog_manager: DialogManager, **middleware_data):
     async with async_session() as session:
        ...
        db_main = set(await session.scalars(select(Catalog.level_4).where(Catalog.level_2 == lvl2_name and Catalog.level_3 == lvl3_name)))
        db_main_items = set(await session.scalars(select(Catalog).where(Catalog.level_2 == lvl2_name and Catalog.level_3 == lvl3_name and Catalog.level_4 == '')))
...
async def get_level_5(dialog_manager: DialogManager, **middleware_data):
     async with async_session() as session:
        ...
        db_main = set(await session.scalars(select(Catalog.level_5).where(Catalog.level_2 == lvl2_name and Catalog.level_3 == lvl3_name and Catalog.level_4 == lvl4_name)))
...
async def get_selected_items(dialog_manager: DialogManager, **middleware_data):
     async with async_session() as session:
        ...
        if level == 'level_3':
            lvl2_name = await session.scalar(select(lvl2_base.level_2).where(lvl2_base.id == int(dialog_manager.current_context().dialog_data.get('level_2'))))
            db_main_items = await session.scalars(select(Catalog).where(Catalog.level_2 == lvl2_name and Catalog.level_3 == ''))
            data = {'item': [(item.name, item.id) for item in db_main_items]}
        elif level == 'level_4':
            lvl2_name = await session.scalar(select(lvl2_base.level_2).where(lvl2_base.id == int(dialog_manager.current_context().dialog_data.get('level_2'))))
            lvl3_name = await session.scalar(select(lvl3_base.level_3).where(lvl3_base.id == int(dialog_manager.current_context().dialog_data.get('level_3'))))
            db_main_items = set(await session.scalars(select(Catalog).where(Catalog.level_3 == lvl3_name and Catalog.level_2 == lvl2_name and Catalog.level_4 == '')))
            data = {'item': [(item.name, item.id) for item in db_main_items]}
        elif level == 'level_5':
            lvl2_name = await session.scalar(select(lvl2_base.level_2).where(lvl2_base.id == int(dialog_manager.current_context().dialog_data.get('level_2'))))
            lvl3_name = await session.scalar(select(lvl3_base.level_3).where(lvl3_base.id == int(dialog_manager.current_context().dialog_data.get('level_3'))))
            lvl4_name = await session.scalar(select(lvl4_base.level_4).where(lvl4_base.id == int(dialog_manager.current_context().dialog_data.get('level_4'))))
            db_main_items = set(await session.scalars(select(Catalog).where(Catalog.level_4 == lvl4_name and Catalog.level_3 == lvl3_name and Catalog.level_5 == '')))
            data = {'item': [(item.name, item.id) for item in db_main_items]}
        else:
            lvl2_name = await session.scalar(select(lvl2_base.level_2).where(lvl2_base.id == int(dialog_manager.current_context().dialog_data.get('level_2'))))
            lvl3_name = await session.scalar(select(lvl3_base.level_3).where(lvl3_base.id == int(dialog_manager.current_context().dialog_data.get('level_3'))))
            ...
            db_main_items = set(await session.scalars(select(Catalog).where(Catalog.level_5 == lvl5_name and Catalog.level_4 == lvl4_name)))
            data = {'item': [(item.name, item.id) for item in db_main_items]}
        return data
