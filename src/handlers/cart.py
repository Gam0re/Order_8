...
async def get_carts(message: types.Message):
    ...
    if len(data) > 0:
        ...
        await message.answer_photo(photo=media['photo'], caption=media['name'], reply_markup=cart_kb(page))
    else:
        await message.answer(text='Корзина пуста', reply_markup=cart_kb('-1'))
...
async def get_carts_next(callback: types.CallbackQuery):
    ...
    if len(data) >= page + 1:
        ...
        await callback.message.edit_media(media=InputMediaPhoto(media=media['photo'], caption=media['name']), reply_markup=cart_kb(page))
    else:
