...
@catalog_router.callback_query(F.data == 'level_2')
async def catalog_lvl3(callback: types.CallbackQuery):
    await callback.answer('Вы выбрали категорию')
    await callback.message.answer('Выберете категорию товара', reply_markup=await cl.catalog_level_3())

@catalog_router.callback_query(F.data == 'level_3')
async def catalog_lvl4(callback: types.CallbackQuery):
    await callback.answer('Вы выбрали категорию')
    await callback.message.answer('Выберете категорию товара', reply_markup=await cl.catalog_level_4())

@catalog_router.callback_query(F.data == 'level_4')
async def catalog_lvl5(callback: types.CallbackQuery):
    await callback.answer('Вы выбрали категорию')
    await callback.message.answer('Выберете категорию товара', reply_markup=await cl.catalog_level_5())
